...
from models import Campus, Membre, Ministere, Organisation, Pole
...
def test_org(session: Session) -> Organisation:
    ...
    org = Organisation(nom=f"Org Test {uuid4()}", date_creation="2024-01-01")
    session.add(org)
    session.flush()
    session.refresh(org)
    return org
...
@pytest.fixture
def test_campus(session: Session, test_org: Organisation) -> Campus:
